...
const standardRateLimiter = t.middleware(async ({ ctx, next }) => {
  const { allowed } = await checkRateLimit(ctx.req, {
    namespace: "standard",
    limit: 100,
    windowSeconds: 300,
  });
...
const strictRateLimiter = t.middleware(async ({ ctx, next }) => {
  ...
  if (!allowed) {
    throw new TRPCError({
      code: "TOO_MANY_REQUESTS",
      message:
        "You are doing that too much. Please try again in 15 minutes and touch some grass!!",
    });
  }
...
const formResponseRateLimiter = t.middleware(async ({ ctx, next }) => {
  ...
  if (!allowed) {
    throw new TRPCError({
      code: "TOO_MANY_REQUESTS",
      message:
        "Too many form responses submitted. Please try again in 15 minutes.",
    });
  }
