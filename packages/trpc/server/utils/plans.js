...
export const PAID_PLAN_IDS = Object.keys(PLAN_DEFINITIONS).filter((planId) => planId !== DEFAULT_PLAN_ID);
...
export function getPlan(planId) {
  return PLAN_DEFINITIONS[resolvePlanId(planId)] ?? PLAN_DEFINITIONS[DEFAULT_PLAN_ID];
}
...
export function getPlanByRazorpayPlanId(razorpayPlanId) {
  return Object.values(PLAN_DEFINITIONS).find((plan) => plan.razorpayPlanId === razorpayPlanId) ?? null;
}
