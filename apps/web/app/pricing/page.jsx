...
export default function PricingPage() {
  ...
  const confirmSubscription =
    trpc.billing.confirmCheckoutSubscription.useMutation();
  ...
  // 🚀 FIX: Correctly read the plan ID from the backend's nested object
  const currentPlanId = subscriptionQuery.data?.plan?.id || "FREE";

  const visiblePlans = useMemo(
    () =>
      planCards.map((plan) => ({
        ...plan,
        selectedPrice: plan.prices[billingCycle],
      })),
    [billingCycle],
  );
  ...
  const handlePlanAction = async (plan) => {
    ...
    try {
      ...
      const razorpay = new window.Razorpay({
        ...
        handler: (response) =>
          confirmCheckout({ planId: selectedPlan.id, response }),
      });
      ...
      razorpay.on("payment.failed", (response) => {
        ...
        toast.error(
          response?.error?.description ||
            "Payment failed. No plan was activated.",
        );
      });
        ...
        <div className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
          <div>
            ...
            <p className="mt-5 max-w-xl text-lg font-medium leading-relaxed text-slate-500">
              Start free, then upgrade to Pro or Business with verified
              recurring billing in Indian Rupees.
            </p>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {[
                ...
                [
                  "Renewal",
                  billingCycle === "yearly" ? "Every year" : "Every month",
                  RefreshCw,
                ],
          ...
          {visiblePlans.map((plan) => {
            ...
            const isCurrentPlan =
              ...
              (currentPlanId === selectedPlan.id ||
                (selectedPlan.id === "FREE" && currentPlanId === "FREE"));
            const isBusy = activePlanId === selectedPlan.id;
            const paidPlanBlocked =
              Boolean(sessionUser) && (paymentScriptFailed || !paymentEnabled);
            const disabled =
              isCurrentPlan ||
              Boolean(activePlanId) ||
              (selectedPlan.id !== "FREE" && paidPlanBlocked);
            ...
            return (
              ...
              >
                ...
                <p className="mt-4 min-h-[3.5rem] text-sm font-medium leading-relaxed text-slate-500">
                  {plan.description}
                </p>
                ...
                <div className="mt-6">
                  ...
                  <div className="flex items-end gap-2">
                    <span className="text-5xl font-medium tracking-tight text-slate-950">
                      {selectedPlan.price}
                    </span>
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-medium tracking-tight text-slate-950">
              Plan limits
            </h2>
          </div>
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {comparisonRows.map((row) => (
            <div
              key={row.label}
              className="grid grid-cols-4 border-t border-slate-200 px-6 py-4 text-sm active:bg-slate-50 transition-colors"
            >
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        <div className="grid gap-6 md:grid-cols-3">
          {[
            [
              "Checkout",
              "Subscription IDs are created on the server before Razorpay opens.",
              CreditCard,
            ],
            [
              "Verification",
              "Payment signatures are checked server-side before activation.",
              ShieldCheck,
            ],
            [
              "Access",
              "Usage limits read from the active subscription plan.",
              BarChart3,
            ],
          ].map(([title, copy, Icon]) => (
            <div
              key={title}
              className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-transform duration-300 active:scale-[1.02]"
            >
