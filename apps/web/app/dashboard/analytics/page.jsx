...
function FormRating({ formId }) {
  ...
  return (
    <div className="mt-2 flex items-baseline gap-1.5">
      <span className="text-lg font-medium text-slate-400 pt-1">
        No reviews
      </span>
    </div>
  );
...
export default function AnalyticsOverviewPage() {
  const { data: forms = [], isLoading } =
    trpc.form.getAnalyticsOverview.useQuery();
    ...
    return forms.reduce(
      (acc, form) => {
        ...
        return {
          ...
          active:
            acc.active +
            (form.isExpired || form.status !== "PUBLISHED" ? 0 : 1),
          latest,
        };
      },
  ...
  return (
    <div className="-m-8 min-h-[calc(100vh-4rem)] bg-[radial-gradient(circle_at_top_right,#dcfce7,transparent_30%),linear-gradient(135deg,#f8fafc,#eef2ff_52%,#fff7ed)] p-4 md:p-8 text-slate-950">
      <div className="mx-auto max-w-7xl space-y-6 md:space-y-8 pt-4">
        ...
        <section className="rounded-[2rem] border border-white/70 bg-white/65 p-6 shadow-xl shadow-slate-200/60 backdrop-blur-xl md:p-8">
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div>
              ...
              <h1 className="text-3xl font-medium text-slate-950 md:text-5xl">
                Response overview
              </h1>
              <p className="mt-3 text-sm md:text-base font-medium text-slate-500 max-w-xl">
                Track conversion rates, review scores, and submission volume
                across all your forms from one unified dashboard.
              </p>
            </div>
            <Link
              ...
              className={cn(
                ...
                "bg-slate-950 text-white active:bg-slate-800 rounded-xl",
              )}
            >
              New form
        ...
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            return (
              <div
                key={stat.label}
                className="animate-rise-in rounded-[1.5rem] border border-white/70 bg-white/65 p-6 shadow-lg shadow-slate-200/60 backdrop-blur-xl transition-transform active:-translate-y-1"
              ...
              >
                ...
                <div
                  className="mt-4 text-2xl md:text-4xl font-medium text-slate-950 truncate"
        ...
        {forms.length === 0 ? (
          <section
            className="animate-rise-in rounded-[2rem] border border-dashed border-slate-300 bg-white/60 p-12 text-center shadow-xl shadow-slate-200/50 backdrop-blur-xl"
          ...
          >
            ...
            <p className="mx-auto mt-3 max-w-md text-sm font-medium leading-6 text-slate-500">
              Create a form and collect a response to see charts, data
              breakdowns, and individual submissions here.
            </p>
            <Link
              href="/dashboard/editor/new"
              className={cn(
                buttonVariants({ size: "lg" }),
                "mt-8 bg-slate-950 text-white active:bg-slate-800 rounded-xl",
              )}
            >
          ...
          <section className="grid gap-5">
            <h2 className="text-xl font-medium text-slate-900 px-2 mt-4">
              Your Forms
            </h2>
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-5">
                  <div className="min-w-0 flex-1">
                    ...
                    <h2 className="truncate text-2xl font-medium text-slate-950">
                      {form.title}
                    </h2>
                  ...
                  <Link
                    ...
                    className={cn(
                      ...
                      "w-full md:w-auto bg-white/80 shadow-sm rounded-xl font-medium border-slate-200 active:bg-slate-50",
                    )}
                  >
                    View Analytics
                    <ArrowRight className="ml-2 size-4" />
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-3">
                  <div className="rounded-xl border border-white/70 bg-white/60 p-4">
                    ...
                    <div className="mt-2 text-3xl font-medium text-slate-950">
