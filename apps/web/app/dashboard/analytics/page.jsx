...
function FormRating({ formId }) {
  ...
  if (reviewStats && reviewStats.totalReviews > 0) {
    return (
      <div className="mt-2 flex items-baseline gap-1.5">
        <span className="text-3xl font-black text-slate-950">{reviewStats.averageRating}</span>
        <span className="text-sm font-semibold text-slate-500">({reviewStats.totalReviews})</span>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 flex items-baseline gap-1.5">
      <span className="text-lg font-bold text-slate-400 pt-1">No reviews</span>
    </div>
  );
}
      ...
      <div className="mx-auto max-w-7xl space-y-6 md:space-y-8 pt-4">
        ...
        <section className="rounded-[2rem] border border-white/70 bg-white/65 p-6 shadow-xl shadow-slate-200/60 backdrop-blur-xl md:p-8">
          <div className="flex flex-wrap items-center justify-between gap-5">
            <div>
              ...
              <h1 className="text-3xl font-black text-slate-950 md:text-5xl">Response overview</h1>
            ...
            <Link href="/dashboard/editor/new" className={cn(buttonVariants({ size: 'lg' }), 'bg-slate-950 text-white hover:bg-slate-800 rounded-xl')}>
              New form
        ...
        <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[
            ...
            return (
              <div key={stat.label} className="animate-rise-in rounded-[1.5rem] border border-white/70 bg-white/65 p-6 shadow-lg shadow-slate-200/60 backdrop-blur-xl transition-transform hover:-translate-y-1" style={{ animationDelay: `${i * 100}ms` }}>
                ...
                <div className="mt-4 text-2xl md:text-4xl font-black text-slate-950 truncate" title={String(stat.value)}>{stat.value}</div>
              </div>
            );
          })}
        </section>
        ...
        {forms.length === 0 ? (
          <section className="animate-rise-in rounded-[2rem] border border-dashed border-slate-300 bg-white/60 p-12 text-center shadow-xl shadow-slate-200/50 backdrop-blur-xl" style={{ animationDelay: '300ms' }}>
            ...
            <h2 className="text-2xl font-black text-slate-950">No analytics yet</h2>
            ...
            <Link href="/dashboard/editor/new" className={cn(buttonVariants({ size: 'lg' }), 'mt-8 bg-slate-950 text-white hover:bg-slate-800 rounded-xl')}>
              Create your first form
            </Link>
          </section>
        ) : (
          <section className="grid gap-5">
            <h2 className="text-xl font-bold text-slate-900 px-2 mt-4">Your Forms</h2>
            {forms.map((form, index) => (
              <article
                key={form.id}
                className="animate-rise-in rounded-[1.5rem] border border-white/70 bg-white/70 p-5 md:p-6 shadow-lg shadow-slate-200/60 backdrop-blur-xl transition-all hover:-translate-y-1 hover:border-emerald-200 hover:shadow-emerald-200/40"
              ...
              >
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-5">
                  <div className="min-w-0 flex-1">
                    <div className="mb-3 flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-white px-3 py-1 text-xs font-bold uppercase tracking-wider text-slate-500 shadow-sm border border-slate-100">
                        {form.visibility}
                      </span>
                      {form.isExpired && (
                        <span className="rounded-full bg-red-100 px-3 py-1 text-xs font-bold uppercase tracking-wider text-red-700 shadow-sm">
                          Closed
                        </span>
                      )}
                    </div>
                    <h2 className="truncate text-2xl font-black text-slate-950">{form.title}</h2>
                  ...
                  <Link
                    ...
                    className={cn(buttonVariants({ variant: 'outline', size: 'lg' }), 'w-full md:w-auto bg-white/80 shadow-sm rounded-xl font-bold border-slate-200 hover:bg-slate-50')}
                  >
                    View Analytics
                    <ArrowRight className="ml-2 size-4" />
                  </Link>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3 lg:grid-cols-3">
                  <div className="rounded-xl border border-white/70 bg-white/60 p-4">
                    ...
                    <div className="mt-2 text-3xl font-black text-slate-950">{form.submissionCount}</div>
                  </div>
                  ...
                  <div className="rounded-xl border border-white/70 bg-white/60 p-4">
                    ...
                    <div className="mt-2 text-3xl font-black text-slate-950">{form.fieldCount}</div>
                  </div>
