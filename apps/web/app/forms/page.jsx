...
export default function PublicFormsPage() {
  const { data, isLoading } = trpc.form.getFormsByRatings.useQuery();
      ...
      <section className="mx-auto max-w-6xl px-4 pb-12 pt-32">
        <div className="rounded-[2rem] border border-white/70 bg-white/65 p-6 shadow-2xl shadow-slate-200/70 backdrop-blur-xl md:p-8">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              <div className="mb-3 inline-flex items-center gap-2 rounded-full bg-amber-100 px-3 py-1.5 text-sm font-medium text-amber-700">
                <Star className="size-4 fill-amber-500" />
                Top Rated Forms
              </div>
              <h1 className="text-4xl font-medium text-slate-950 md:text-6xl">
                Hall of Fame
              </h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
                Browse the top 5 highest-rated published forms from our
                creators.
              </p>
            </div>
            <Link
        ...
        {isLoading ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white/60 p-12 text-center shadow-xl shadow-slate-200/60 backdrop-blur-xl">
            ...
            <p className="mx-auto mt-3 max-w-md text-sm leading-6 text-slate-600">
              Publish a form from the dashboard and it will appear here for
              anyone to answer.
            </p>
          </div>
        ) : (
