        ...
        <div className="rounded-[2rem] border border-white/70 bg-white/65 p-6 shadow-2xl shadow-slate-200/70 backdrop-blur-xl md:p-8">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <div>
              ...
              <h1 className="text-4xl font-medium text-slate-950 md:text-6xl">View forms</h1>
              <p className="mt-4 max-w-2xl text-base leading-7 text-slate-600">
                Browse published forms from creators and answer the ones that matter to you.
              </p>
            </div>
            <Link href="/dashboard/editor/new" className={cn(buttonVariants({ size: 'lg' }), 'bg-slate-950 text-white active:bg-slate-800')}>
              Publish yours
      ...
      <section className="mx-auto max-w-6xl px-4 pb-20">
        {isLoading ? (
          ...
          <div className="rounded-[2rem] border border-dashed border-slate-300 bg-white/60 p-12 text-center shadow-xl shadow-slate-200/60 backdrop-blur-xl">
            ...
            <h2 className="text-2xl font-medium text-slate-950">No public forms yet</h2>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {forms.map((form, index) => (
                ...
                >
                  ...
                  <h2 className="line-clamp-2 text-xl font-medium text-slate-950">{form.title}</h2>
                  ...
                  <div className="mt-5 grid grid-cols-2 gap-2">
                    <div className="rounded-xl bg-white/70 p-3">
                      <div className="text-xs font-medium uppercase text-slate-400">Fields</div>
                      <div className="mt-1 text-xl font-medium text-slate-950">{form.fieldCount}</div>
                    </div>
                    <div className="rounded-xl bg-white/70 p-3">
                      ...
                      <div className="mt-1 text-xl font-medium text-slate-950">{form.submissionCount}</div>
                    </div>
                  </div>

                  <Link href={`/forms/${form.slug}`} className={cn(buttonVariants({ size: 'lg' }), 'mt-5 w-full bg-slate-950 text-white active:bg-slate-800')}>
                    Answer form
                    <ArrowRight className="ml-2 size-4" />
                  </Link>
                </article>
